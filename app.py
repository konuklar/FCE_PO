...
# Create mock classes first for fallback
class MockEfficientFrontier:
    ...
    def max_sharpe(self, *args, **kwargs):
        return {}
    def max_quadratic_utility(self, *args, **kwargs):
        return {}
    def efficient_risk(self, *args, **kwargs):
        return {}
    def efficient_return(self, *args, **kwargs):
        return {}
    def portfolio_performance(self, *args, **kwargs):
        return 0, 0, 0
    def clean_weights(self, *args, **kwargs):
        return {}
    def add_objective(self, *args, **kwargs):
        pass

# Import PyPortfolioOpt with fallbacks
if PYPFOPT_AVAILABLE:
    try:
        # Import all PyPortfolioOpt components with enhanced error handling
        from pypfopt import expected_returns, risk_models
        from pypfopt.efficient_frontier import EfficientFrontier as PFOpt_EfficientFrontier
        ...
        # Use the real EfficientFrontier
        EfficientFrontier = PFOpt_EfficientFrontier
        PYPFOPT_AVAILABLE = True
...
class EnhancedInstitutionalPortfolioOptimizer:
    ...
    def optimize_with_constraints(self, returns_df: pd.DataFrame, strategy: str,
        ...
        if risk_free_rate is None:
            risk_free_rate = InstitutionalConfig.DEFAULT_RF_RATE
    ...
    def _validate_optimization_inputs(self, returns_df: pd.DataFrame, strategy: str):
        ...
        if len(returns_df.columns) < InstitutionalConfig.MIN_ASSETS:
            raise ValueError(f"Need at least {InstitutionalConfig.MIN_ASSETS} assets, got {len(returns_df.columns)}")
        
        if len(returns_df) < InstitutionalConfig.MIN_DATA_POINTS:
            raise ValueError(f"Need at least {InstitutionalConfig.MIN_DATA_POINTS} data points, got {len(returns_df)}")
    ...
    def _default_constraints(self, returns_df: pd.DataFrame) -> Dict:
        ...
        return {
            ...
            'max_weight': 1.0,
            'sum_to_one': True,
            'min_diversification': InstitutionalConfig.MIN_DIVERSIFICATION,
            'max_concentration': InstitutionalConfig.MAX_CONCENTRATION,
            'max_leverage': InstitutionalConfig.MAX_LEVERAGE,
            'allow_short': False,
            'asset_groups': None,
            'sector_limits': None,
            'turnover_limit': None,
            'liquidity_threshold': InstitutionalConfig.LIQUIDITY_THRESHOLD,
            'max_drawdown': InstitutionalConfig.MAX_DRAWDOWN,
            'risk_aversion': 1.0,
            'target_risk': None,
            'target_return': None,
            'reg_gamma': InstitutionalConfig.REGULARIZATION_GAMMA,
            'max_iterations': InstitutionalConfig.MAX_ITERATIONS,
            'tolerance': InstitutionalConfig.OPTIMIZATION_TOLERANCE
        }
    ...
    def _optimize_with_pypfopt(self, returns_df: pd.DataFrame, strategy: str,
        ...
        try:
            ...
            expected_return, expected_risk, sharpe_ratio = ef.portfolio_performance(
                risk_free_rate=risk_free_rate / InstitutionalConfig.TRADING_DAYS,
                verbose=False
            )
            
            # Convert to annualized
            expected_return_annual = expected_return * InstitutionalConfig.TRADING_DAYS
            expected_risk_annual = expected_risk * np.sqrt(InstitutionalConfig.TRADING_DAYS)
            sharpe_ratio_annual = sharpe_ratio * np.sqrt(InstitutionalConfig.TRADING_DAYS)
    ...
    def _apply_pypfopt_strategy(self, ef: EfficientFrontier, strategy: str,
        ...
        daily_rf = risk_free_rate / InstitutionalConfig.TRADING_DAYS
        ...
        if strategy == "Minimum Volatility":
            ...
            return ef.efficient_risk(target_risk=target_risk / np.sqrt(InstitutionalConfig.TRADING_DAYS))
        
        elif strategy == "Efficient Return":
            target_return = constraints.get('target_return', 0.10)
            return ef.efficient_return(target_return=target_return / InstitutionalConfig.TRADING_DAYS)
    ...
    def _optimize_most_diversified(self, returns_df: pd.DataFrame,
                                  constraints: Dict, risk_free_rate: float) -> Dict:
        """Most diversified portfolio optimization"""
        return self._optimize_max_diversification(returns_df, constraints, risk_free_rate)
    ...
    def _optimize_custom_base(self, returns_df: pd.DataFrame, method: str,
        ...
        # Calculate expected returns and covariance
        mu = returns_df.mean().values * InstitutionalConfig.TRADING_DAYS
        Sigma = returns_df.cov().values * InstitutionalConfig.TRADING_DAYS
    ...
    def _solve_min_volatility(self, Sigma: np.ndarray, constraints: Dict) -> np.ndarray:
        ...
        result = optimize.minimize(objective, w0, method='SLSQP', 
                         ...
                         options={'maxiter': InstitutionalConfig.MAX_ITERATIONS,
                                  'ftol': InstitutionalConfig.OPTIMIZATION_TOLERANCE})
    ...
    def _solve_max_utility(self, mu: np.ndarray, Sigma: np.ndarray, 
                          risk_aversion: float, constraints: Dict) -> np.ndarray:
        """Solve maximum utility portfolio"""
        n = len(mu)
    ...
    def _solve_efficient_risk(self, mu: np.ndarray, Sigma: np.ndarray, 
                             target_risk: float, constraints: Dict) -> np.ndarray:
        """Solve efficient risk portfolio"""
        n = len(mu)
        ...
        if result.success:
            ...
            return self._solve_min_volatility(Sigma, constraints)
    
    def _solve_efficient_return(self, mu: np.ndarray, Sigma: np.ndarray, 
                               target_return: float, constraints: Dict) -> np.ndarray:
        """Solve efficient return portfolio"""
        n = len(mu)
        ...
        if result.success:
            ...
            # Fallback to maximum return with constraints
            w = np.zeros(n)
    ...
    def _optimize_risk_parity(self, returns_df: pd.DataFrame,
        ...
        cov_matrix = returns_df.cov().values * InstitutionalConfig.TRADING_DAYS
    ...
    def _optimize_hrp(self, returns_df: pd.DataFrame,
        ...
        if PYPFOPT_AVAILABLE:
            try:
                ...
                # Fallback to inverse volatility
                volatilities = returns_df.std().values * np.sqrt(InstitutionalConfig.TRADING_DAYS)
                weights = 1 / (volatilities + 1e-10)
                weights = weights / weights.sum()
        else:
    ...
    def _apply_risk_limits(self, result: Dict, returns_df: pd.DataFrame) -> Dict:
        ...
        if max_weight > InstitutionalConfig.MAX_CONCENTRATION:
            logger.warning(f"Concentration limit exceeded: {max_weight:.2%} > {InstitutionalConfig.MAX_CONCENTRATION:.2%}")
            
            # Apply concentration limit
            excess = max_weight - InstitutionalConfig.MAX_CONCENTRATION
        ...
        effective_n = self._calculate_effective_n(weights)
        if effective_n < InstitutionalConfig.MIN_DIVERSIFICATION:
            logger.warning(f"Diversification below minimum: {effective_n:.1f} < {InstitutionalConfig.MIN_DIVERSIFICATION}")
            
            # Try to improve diversification by reducing largest weights
            sorted_indices = np.argsort(weights)[::-1]
            while effective_n < InstitutionalConfig.MIN_DIVERSIFICATION and len(sorted_indices) > 1:
    ...
    def _apply_compliance_checks(self, result: Dict, returns_df: pd.DataFrame,
        ...
        if leverage > constraints.get('max_leverage', InstitutionalConfig.MAX_LEVERAGE):
            compliance_issues.append(f"Leverage {leverage:.2f} exceeds limit {constraints['max_leverage']}")
        
        # Check volatility
        if result['expected_risk'] > InstitutionalConfig.MAX_VOLATILITY:
            compliance_issues.append(f"Volatility {result['expected_risk']:.2%} exceeds limit {InstitutionalConfig.MAX_VOLATILITY:.2%}")
        
        # Check Sharpe ratio
        if result['sharpe_ratio'] < InstitutionalConfig.MIN_SHARPE:
            compliance_issues.append(f"Sharpe ratio {result['sharpe_ratio']:.2f} below minimum {InstitutionalConfig.MIN_SHARPE}")
    ...
    def _calculate_additional_metrics(self, result: Dict, returns_df: pd.DataFrame,
        ...
        downside_dev = downside_returns.std() * np.sqrt(InstitutionalConfig.TRADING_DAYS) if len(downside_returns) > 0 else 0
        ...
        tracking_error = active_returns.std() * np.sqrt(InstitutionalConfig.TRADING_DAYS)
        information_ratio = active_returns.mean() * InstitutionalConfig.TRADING_DAYS / (tracking_error + 1e-10)
    ...
    def _fallback_optimization(self, returns_df: pd.DataFrame, 
        ...
        portfolio_returns = (returns_df * weights).sum(axis=1)
        expected_return = portfolio_returns.mean() * InstitutionalConfig.TRADING_DAYS
        expected_risk = portfolio_returns.std() * np.sqrt(InstitutionalConfig.TRADING_DAYS)
...
class EnhancedInstitutionalRiskAnalytics:
    ...
    def _compute_basic_risk_metrics(self, portfolio_returns: pd.Series,
        ...
        annual_factor = InstitutionalConfig.TRADING_DAYS
        ...
        threshold = risk_free_rate / InstitutionalConfig.TRADING_DAYS
    ...
    def _ewma_var(self, returns: pd.Series, confidence: float = 0.95) -> float:
        """Exponentially Weighted Moving Average VaR"""
        # Simple implementation - returns historical VaR
        return self._historical_var(returns, confidence)
    
    def _monte_carlo_var(self, returns: pd.Series, confidence: float = 0.95) -> float:
        """Monte Carlo VaR"""
        # Simple implementation - returns historical VaR
        return self._historical_var(returns, confidence)
    
    def _garch_var(self, returns: pd.Series, confidence: float = 0.95) -> float:
        """GARCH VaR"""
        # Simple implementation - returns historical VaR
        return self._historical_var(returns, confidence)
        ...
        for scenario_name, scenario_params in self.stress_scenarios.items():
            try:
                ...
                if avg_daily_return > 0:
                    recovery_days = abs(total_loss) / (avg_daily_return * InstitutionalConfig.TRADING_DAYS / 12)
                else:
    ...
    def _apply_stress_scenario(self, returns_df: pd.DataFrame,
        ...
        for ticker in returns_df.columns:
            ...
            daily_shock = shock / InstitutionalConfig.TRADING_DAYS
    ...
    def _compute_liquidity_metrics(self, returns_df: pd.DataFrame,
        ...
        liquidity_metrics.update({
            ...
            'illiquid_assets_pct': (portfolio_weights > InstitutionalConfig.LIQUIDITY_THRESHOLD).sum() / len(portfolio_weights)
        })
...
class InstitutionalPortfolioApplication:
    ...
    def _initialize_session_state(self):
        ...
        if 'initialized' not in st.session_state:
            ...
            st.session_state.risk_free_rate = InstitutionalConfig.DEFAULT_RF_RATE
    ...
    def _render_sidebar(self):
        ...
        all_tickers = all_tickers[:InstitutionalConfig.MAX_ASSETS]
        ...
        with st.sidebar.expander("⚙️ Advanced Settings"):
            st.session_state.risk_free_rate = st.number_input(
                ...
                min_value=0.0,
                max_value=20.0,
                value=InstitutionalConfig.DEFAULT_RF_RATE * 100,
                step=0.1
            ) / 100
    ...
    def _render_key_metrics(self):
        ...
        # Calculate basic statistics
        annual_returns = returns_df.mean() * InstitutionalConfig.TRADING_DAYS
        annual_volatility = returns_df.std() * np.sqrt(InstitutionalConfig.TRADING_DAYS)
    ...
    def _render_asset_performance_table(self):
        ...
        for ticker in returns_df.columns:
            ...
            # Basic metrics
            annual_return = asset_returns.mean() * InstitutionalConfig.TRADING_DAYS
            annual_vol = asset_returns.std() * np.sqrt(InstitutionalConfig.TRADING_DAYS)
            ...
            downside_vol = downside_returns.std() * np.sqrt(InstitutionalConfig.TRADING_DAYS) if len(downside_returns) > 0 else 0
    ...
    def _plot_efficient_frontier(self):
        ...
        try:
            ...
            # Individual assets
            asset_risks = np.sqrt(np.diag(S * InstitutionalConfig.TRADING_DAYS))
            asset_returns = mu.values * InstitutionalConfig.TRADING_DAYS
    ...
    def _render_rolling_analysis(self):
        ...
        if metric == 'Volatility':
            rolling_metric = portfolio_returns.rolling(window=window_size).std() * np.sqrt(InstitutionalConfig.TRADING_DAYS)
            yaxis_title = "Annualized Volatility"
            format_func = lambda x: f"{x:.2%}"
        elif metric == 'Sharpe Ratio':
            rolling_returns = portfolio_returns.rolling(window=window_size).mean() * InstitutionalConfig.TRADING_DAYS
            rolling_vol = portfolio_returns.rolling(window=window_size).std() * np.sqrt(InstitutionalConfig.TRADING_DAYS)
            rolling_metric = (rolling_returns - st.session_state.risk_free_rate) / rolling_vol
            yaxis_title = "Sharpe Ratio"
            format_func = lambda x: f"{x:.2f}"
        elif metric == 'Returns':
            rolling_metric = portfolio_returns.rolling(window=window_size).mean() * InstitutionalConfig.TRADING_DAYS
    ...
    def _render_data_export(self):
        ...
        if st.session_state.optimization_results:
            ...
            with col2:
                # Export optimization report
                report_text = self._generate_optimization_report(st.session_state.optimization_results)
                st.download_button(
                    label="📄 Download Optimization Report",
                    data=report_text,
                    file_name="optimization_report.txt",
        ...
        if st.session_state.risk_metrics:
            ...
            with col1:
                # Export risk metrics
                risk_df = pd.DataFrame([st.session_state.risk_metrics])
            ...
            with col2:
                # Export risk report
                risk_report = self.risk_analytics.generate_risk_report(st.session_state.risk_metrics)
                st.download_button(
                    label="📄 Download Risk Report",
                    data=risk_report,
                    file_name="risk_report.txt",
    ...
    def _render_security_logs(self):
        """Render security and logs interface"""
        ...
        with st.form("security_settings"):
            ...
            with col1:
                ...
                enable_compliance = st.checkbox("Enable Compliance Checks", value=True)
            
            with col2:
                enable_risk_limits = st.checkbox("Enable Risk Limits", value=True)
                max_login_attempts = st.number_input("Max Login Attempts", min_value=1, max_value=10, value=3)
        ...
        log_files = ['institutional_portfolio.log']
        ...
        if st.button("View Logs"):
            try:
                ...
                st.warning(f"Log file '{selected_log}' not found.")
            except Exception as e:
        ...
        if st.button("Clear Logs", type="secondary"):
            try:
                with open('institutional_portfolio.log', 'w') as f:
                    f.write('')
    ...
    def _render_footer(self):
        ...
        with col2:
            st.markdown("**📊 Professional Portfolio Optimization**")
            st.markdown("Global Multi-Asset Edition")
        
        with col3:
            st.markdown("**🔧 Enhanced Institutional Features**")
            st.markdown("Comprehensive Error Handling")
        
        st.markdown("---")
        st.markdown("© 2024 Institutional Apollo / ENIGMA. All rights reserved.")
